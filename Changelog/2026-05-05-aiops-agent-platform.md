...
- 修复 AIOps 最终报告前端渲染：重写 `static/app.js` 的 AIOps Markdown 组装、Trace 按钮文案、完成态渲染与反馈提示挂载逻辑，避免最终报告阶段的标题错乱、乱码文案和因损坏字符串导致的脚本语法错误。
...
- 2026-05-15：修复 AIOps 流式诊断的 Trace / 报告时序问题：后端在 planner / replanner 生成 response 时不再直接对外声明‘最终报告’，而是统一发送 report_draft 候选报告事件；前端区分‘候选报告（等待 Verifier 校验）’与‘最终报告’，只有在 complete 事件到达后才持久化 aiops-final 并显示反馈区，同时在前端加入 complete 后的状态收口，避免界面表现成‘流程结束后 Trace 仍在继续增长’。
- 2026-05-15：收口 `remote_host` 磁盘诊断链路：`app.monitoring.monitor_provider` 统一改为通过 `app.config.config` 读取 `AIOPS_MONITOR_PROVIDER`、`AIOPS_REMOTE_HOST_BASE_URL`、`AIOPS_REMOTE_HOST_TOKEN`，并把远程鉴权头修正为 `X-Host-Agent-Token`；`disk_cleanup` 报告不再硬编码 `demo-server-01`，会优先使用工具返回的真实主机名，并按 `source=mock|remote_host` 动态说明证据来源。对于仍未接入实时采集的 `list_large_files`、`query_deleted_open_files`、`get_disk_cleanup_candidates`，在 `remote_host` 模式下不再混入 mock 结果，而是明确显示“暂未接入实时工具”，避免把 Demo 数据伪装成真实现场证据；同时补充了 provider 切换、远程成功/失败适配和磁盘报告的最小回归测试。
- 2026-05-15：继续补齐 `remote_host` 磁盘诊断实时证据：`list_large_files` 与 `query_deleted_open_files` 已接入 Ubuntu Host Agent 的 `/api/v1/disk/large-files` 与 `/api/v1/disk/deleted-open-files`，并在 provider 层完成 `ok=false`、超时、非 200、非法 JSON 的统一结构化兜底。`disk_cleanup` 已适配 `scan_incomplete`、`permission_denied_count`、`filtered_out_count`、`filters_applied` 等字段，报告会区分“工具失败”“未发现高价值结果”“过滤掉噪声记录”，同时在 `remote_host` 模式下继续隐藏仍为 mock 的 `get_disk_cleanup_candidates` 数据，避免把假数据混入真实诊断报告。
- 2026-05-16ï¼šå®Œæˆ AIOps Investigation Engine ç¬¬ä¸€é˜¶æ®µé‡æž„åŸºç¡€è®¾æ–½ï¼šæ–°å¢ž `app/agent/aiops/investigation/` æ¨¡å—ï¼Œå®šä¹‰ `DiagnosisIntent`ã€`DiagnosisProfile`ã€`EvidenceRecord`ã€`InvestigationTask`ã€`StopDecision` ç­‰åŸºç¡€æ¨¡åž‹ï¼Œå¹¶åœ¨ `PlanExecuteState` ä¸­è¡¥å……… `diagnosis_intent`ã€`selected_profile`ã€`evidence_store`ã€`investigation_round`ã€`no_progress_rounds`ã€`stop_decision` å­—æ®µï¼Œä¸ºåŽç»­ç»Ÿä¸€ Investigation Engine è¿ç§»é¢„å…ˆæ‰“åº•ã€‚
- 2026-05-16ï¼šé‡å®šä½ Skill ç”¨é€”å¹¶é™çº§æ—§ Skillï¼š`SkillDefinition` æ–°å¢ž `skill_mode` / `profile_id`ï¼Œæœªå£°æ˜Ž `skill_mode` çš„æ—§ `SKILL.md` é»˜è®¤æŒ‰ `reference_playbook` å¤„ç†ï¼Œ`draft` ä¸å†è¿›å…¥æ­£å¼æ‰§è¡Œé“¾è·¯ï¼Œå½“å‰åªå°† `disk_cleanup` æå‡ä¸º `execution_profile`ã€‚
- 2026-05-16ï¼šå¯¹ custom AIOps åšç¬¬ä¸€é˜¶æ®µæ­¢è¡€ï¼šæ–°å¢ž `AIOPS_ALLOW_LEGACY_GENERIC_DIAGNOSIS=false`ï¼Œé»˜è®¤å…³é—­æ—§ generic è‡ªä¸»è¯Šæ–­é•¿é“¾ã€‚å½“ custom è¯·æ±‚æœªå‘½ä¸­ `execution_profile` æ—¶ï¼ŒPlanner ä¸å†è¿›å…¥ generic plan + LLM fallbackï¼Œè€Œæ˜¯ç›´æŽ¥è¾“å‡ºå—æŽ§ç»“æžœè¯´æ˜Žå¹¶ç»“æŸï¼Œé¿å…è¯æ®ä¸è¶³æ—¶çš„è¿‡åº¦æŽ¨æ–­åž‹è¯Šæ–­ã€‚
- 2026-05-16ï¼šVerifier / Replanner ä¸å†å¯¹ legacy generic è·¯å¾„å›žå¡«è‡ªç”±æ–‡æœ¬ planï¼šå¯¹ `generic_llm`ã€`generic_template_fallback`ã€`controlled_no_profile` ç­‰éžç»“æž„åŒ– legacy åˆ†æ”¯ï¼ŒVerifier å¦‚æžœä¸é€šè¿‡ï¼Œåªä¼šè¾“å‡º `finalize_with_limitations` çš„ `stop_decision`ï¼Œä¸å†æŠŠ `suggested_next_steps` ç›´æŽ¥å¡žå›ž plan é€ æˆ Trace è†¨èƒ€å’Œæ‰§è¡Œé•¿é“¾å¾ªçŽ¯ã€‚
